...
PGHOME="/iczin-guru/app/postgres/product/${PG_VERSION}"
...
{
    echo Starting create ${PGUSR} with ${PGDDB} context

    ${PGHOME}/bin/psql -c "select version()" postgres
    ${PGHOME}/bin/psql -c "create user ${PGUSR} with login password '${PGPASSWD}'" postgres
    ${PGHOME}/bin/psql -c "create database ${PGDDB} with owner = ${PGUSR}" postgres
}
...
_pg_start()
{   
    echo "Waiting for connection !!! Sucsess !!!"
